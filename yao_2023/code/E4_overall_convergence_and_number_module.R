...
processed_results <- fread(glue(work_directory, '/yao_2023/data/intermediate_data/', batch_name, '/E3_processed_results.csv'))
...
plot_path <- file.path(work_directory, 
                    "paper_plots", "data", paste0("E4_",batch_name, "_overall_significance.pdf"))
ggsave(filename = plot_path, plot = p, width = 9, height = 6*9/8, bg = "transparent")

dir.create(file.path(work_directory, 
                  "yao_2023/report/"), recursive = TRUE)
plot_path <- file.path(work_directory, 
                    "yao_2023", "report", paste0("E4_",batch_name, "_overall_significance.pdf"))
ggsave(filename = plot_path, plot = p, width = 9, height = 6*9/8, bg = "transparent")
