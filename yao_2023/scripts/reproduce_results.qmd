...
As a sensitivity analysis, we restricted the feature set to genes from the immunity-relevant modules and reran XConTest using lasso rather than group lasso as the classifier. In this analysis, selected features are individual genes rather than gene modules.
...
source(file.path(code_directory, 'G5_examine_important_pairs.R'),
        local = TRUE) # it replicates the old results
```
